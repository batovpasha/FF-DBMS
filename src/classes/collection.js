...
class Collection{ // a class that describes the structure and behavior of the collection
  ...
  hashFunction(key, password){ // a function that generates a hash value for a given key
    const cipher = crypto.createCipher('aes192', password);
    let hash = cipher.update(key, 'utf8', 'hex');
  ...
  insertItem(item, password){ // method of inserting an element into a hash table
    if (this.itemSchema.validityCheck(item)){
      let key = '';
      for (let i of this.keySchema) key += item[i].toString();
      let hash = this.hashFunction(key, password);
  ...
  updateItem(item, password){ // method of updating the value according to the given key
    if (this.itemSchema.validityCheck(item)){
      let queryItem = this.findItem(item, password);
      for (let i in item) queryItem[i] = item[i];
    }
  }

  drop(password){ // method of cleaning the collection
